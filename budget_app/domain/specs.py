...
로 조합한다. 조건을 추가하는 일이 **클래스 하나 추가**가 되고 기존 코드는 그대로다.

    spec = DateRange("2024-01-01", "2024-01-31") & (InCategory("food") | InCategory("cafe")) & ~HasTag("정기")

## 왜 이 프로젝트에 맞나

``SearchFilter`` 를 ``list``/``search``/``summary``/``export`` 넷이 공유한다. 조건
표현력이 한 번 늘면 네 명령이 동시에 강해진다.
...
def date_range(start: str, end: str) -> Spec:
    """``DateFrom(start) & DateTo(end)`` 의 축약."""
