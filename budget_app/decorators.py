...
def handle_errors(func: Callable[..., int]) -> Callable[..., int]:
    ...
    def wrapper(*args, **kwargs) -> int:
        ...
        # ---------- (1) 종료 신호 — 오류가 아님 ----------
            ...
            output.err(config.MSG_INTERRUPTED)
            return config.EXIT_INTERRUPT

        # ---------- (2) 입력 오류 — 사용자가 값을 고치면 해결됨 ----------
        except ValidationError as exc:
            output.err(config.MSG_ERROR_LINE.format(msg=exc))
            output.err(config.HINT_VALIDATION)
            return config.EXIT_VALIDATION
        except AppError as exc:
            output.err(config.MSG_ERROR_LINE.format(msg=exc.message))
            if exc.hint:
                output.err(config.MSG_HINT_LINE.format(msg=exc.hint))
            return config.EXIT_APP

        # ---------- (3) 환경 상태 — 파일/권한/인코딩/디스크 ----------
        except FileNotFoundError as exc:
            output.err(config.MSG_ERR_FILE_NOT_FOUND.format(name=exc.filename or exc))
            output.err(config.HINT_FILE_NOT_FOUND)
            return config.EXIT_IO
        except IsADirectoryError as exc:
            output.err(config.MSG_ERR_IS_A_DIR.format(name=exc.filename or exc))
            output.err(config.HINT_IS_A_DIR)
            return config.EXIT_IO
        except PermissionError as exc:
            output.err(config.MSG_ERR_PERMISSION.format(name=exc.filename or exc))
            output.err(config.HINT_PERMISSION)
            return config.EXIT_IO
        except UnicodeDecodeError:
            output.err(config.MSG_ERR_ENCODING)
            output.err(config.HINT_ENCODING)
            return config.EXIT_ENCODING
        except OSError as exc:
            # 디스크 가득 참(ENOSPC), 파일 잠금 등 위에서 못 잡은 입출력 오류.
            output.err(config.MSG_ERR_IO.format(error=exc))
            output.err(config.HINT_IO)
            return config.EXIT_IO
        ...
        # ---------- (4) 최후 방어선 — 분류 밖의 버그 ----------
            ...
            output.err(config.MSG_ERR_UNEXPECTED.format(error=exc))
            output.err(config.HINT_UNEXPECTED)
