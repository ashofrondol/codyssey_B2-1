...
- [x] 0-1. `parse_date('2024-1-5')` → `'2024-01-05'` 정규화 기대 / 비패딩 날짜 거래가 월 요약에 포함되는지
- [x] 0-2. `budget set 2024-1` 후 `summary 2024-01`에서 예산이 보이는지
- [x] 0-3. 최대 번호 거래 삭제 후 add 시 **다른 ID**가 발급되는지
- [x] 0-4. `remove('etc', replace_with=' etc ')`가 차단/정규화되어 고아 거래가 안 생기는지
- [x] 0-5. `TransactionId('TX-1')`과 `TransactionId('TX-000001')`이 동일 취급되는지
- [x] 0-6. 개행 없이 잘린 꼬리를 가진 jsonl에 append 시 새 레코드가 살아남는지
- [x] 0-7. UTF-8 BOM 붙은 CSV import 성공 여부
- [x] 0-8. `--data-dir <기존 파일 경로>` 실행 시 트레이스백 없이 친절한 오류 + EXIT_IO(3)
- [x] 0-9. `list --limit 0` 이 "(데이터 없음)"을 출력하지 않는지 (오류 또는 전체 출력)
- [x] 0-10. 바이트 손상(`\xff`) 줄 하나가 파일 전체 읽기를 죽이지 않는지
- [x] 정상 경로 스모크 테스트: add→list→search→summary→export→import 왕복

## Phase 1 — 조용한 데이터 손상 버그 (최우선)

- [x] 1-1. **날짜/월 정규화** — `domain/validators.py:58,67`
...
- [x] 1-2. **TransactionId 정규화(별칭 제거)** — `domain/config.py:25`, `domain/tx_id.py`
...
- [x] 1-3. **삭제된 최대 ID 재사용 방지** — `storage/repositories.py:52-71`, `storage/ids.py`
...
- [x] 1-4. **카테고리 비교 정규화 통일** — `services/categories.py:37-61`, `storage/repositories.py:102,164-180`
...
- [x] 2-1. **append 꼬리 개행 확인** — `storage/jsonl.py:181-194`
  append 전 마지막 바이트가 `\n`인지 확인(`rb`로 seek), 아니면 개행을 먼저 쓴다. (찢어진 꼬리에 새 레코드가 병합·소실되던 버그)
- [x] 2-2. **append 경로 fsync** — `storage/jsonl.py:181-194`
  재작성 경로만 fsync하는 내구성 비대칭 해소: `append`/`append_all`에도 flush+fsync 추가 (CLI 단건 쓰기라 비용 무시 가능).
- [x] 2-3. **UnitOfWork.commit 실패 후처리** — `storage/unit_of_work.py:77-91`
...
- [x] 2-4. **바이트 손상 라인 격리** — `storage/jsonl.py:138-150`
...
- [x] 2-5. **CSV 읽기 utf-8-sig** — `storage/csv_io.py:83`, `storage/config.py:30`
  읽기 인코딩만 `utf-8-sig`로 (BOM 없는 파일에도 무해). 쓰기는 BOM 없는 utf-8 유지(왕복 안전성 주석 근거 유지).
- [x] 2-6. ⚖️ **태그 CSV 왕복 무손실** — `storage/csv_io.py:159`
  쉼표 포함 태그가 join/split에서 분해된다. 권고: 태그 검증(`parse_tags`)에서 구분자 포함을 거부하는 쪽이 단순(CSV 포맷 불변). 대안: 이스케이프 도입(포맷 변경).
- [x] 2-7. **`append_many(atomic=True)` 죽은 분기 제거** — `storage/repositories.py:106-119`
  UoW 도입 후 원자 커밋 메커니즘이 2개다. atomic 플래그를 제거하고 호출부를 정리.
- [~] 2-8. ⚖️ **동시 실행 방어(선택)** — *스코프 아웃, README 에 전제 명시* — `storage/jsonl.py:59`
...
- [x] 4-1. **CLI→storage 우회 3곳 제거** — `cli/prompts.py:20`, `cli/handlers.py:21,28,34`
...
- [x] 4-2. **AppContext 저장소 비공개화** — `context.py:42-48`
  `txs/cats/budgets` → `_txs/_cats/_budgets`, 핸들러에는 서비스만 노출. (4-1 완료가 선행 조건)
- [x] 4-3. **ImportReport에서 표시 문자열 제거** — `domain/results.py:73-74`, `services/importexport.py:51-61`
...
- [x] 4-4. **config/messages 자기 규칙 위반 정리** — `storage/unit_of_work.py:90`, `budget_app/config.py`
...
- [x] 5-1. ⚖️ **Specification 패턴 처리** — `domain/specs.py`
...
- [x] 5-2. **UoW 문서 정직화** — `storage/unit_of_work.py`
...
- [x] 5-3. **이중 전체 스캔 제거 + 존재 판정 통일** — `storage/repositories.py:121-180`
...
- [x] 5-4. **엔티티 frozen 전환** — `domain/entities.py`
...
- [x] 6-3. **잔여 검증 구멍** — `domain/validators.py`
...
- [x] 6-5. **docs/ 12편 라인 번호 일괄 갱신** — 모든 소스 수정 완료 후 마지막에.
- [x] 6-6. **전체 테스트 + 스모크 실행**으로 마무리 검증.

---

## 완료 보고 (2026-08-07)

Phase 0~6 전부 반영. **Phase 단위로 7개 커밋**(`393a806`~`e7db755`)이며, 각 커밋
메시지에 "무엇을 왜 고쳤는가"가 들어 있다.

### 최종 검증

| 항목 | 결과 |
|---|---|
| pytest | **136/136 통과** (Phase 0 의 64개 + 아키텍처 47개 + 패턴 25개) |
| ruff (자체 설정) | **All checks passed** — 시작 시점 175건 |
| 문서 소스 인용 | 494개 — 파일·라인 오류 0, 본문 불일치 0 |
| 문서 내부 링크 | 201개 — 오류 0 |
| CLI 스모크 | 11개 명령 실행 확인 |

### ⚖️ 항목 결정

| 항목 | 결정 | 근거 |
|---|---|---|
| 2-6 태그 CSV 왕복 | **권고안 채택** — `parse_tags` 에서 구분자 포함 거부 | 교환 포맷을 그대로 두는 쪽이 단순 |
| 2-8 동시 실행 방어 | **스코프 아웃** — README 에 "단일 프로세스 전제" 명시 | `.tmp` 고유화는 단일 프로세스에서 얻는 것이 없고 POSIX 파일 권한만 0600 으로 바꾼다. 잠금 파일만 넣으면 "비정상 종료 후 남은 잠금" 이라는 새 고장을 들여온다. 필요해지면 되돌릴 수 있다 |
| 5-1 Specification | **권고안 채택** — 유지하되 허위 주장 정정, 죽은 `date_range` 삭제 | 과제가 패턴 시연 목적이고, "조합 대수는 선행 투자" 라고 정직하게 적는 편이 방어에 유리 |
| 5-2 UoW 개명 | **개명하지 않음** — docstring 에 "Fowler 식 UoW 가 아니라 staged commit" 명시 | 해결하는 문제가 같아 이름은 유지할 만하고, 경계는 문서로 못 박는 편이 낫다 |
| 4-4 상수 모듈 9개 통합 | **유지** | 계층별 소유가 이 프로젝트의 서사이고, 실측(46개 중 39개가 단일 계층)이 그 분할을 뒷받침한다 |

### 계획에 없었지만 함께 고친 것

- `--data-dir` 가 파일을 가리킬 때 "파일이 이미 있으므로 만들 수 없습니다" 대신
  원인을 말하는 오류 (`NotADirectoryError` → 종료 코드 3)
- 백업이 `*.jsonl` 만 복사해 새 `id_counter` 를 빠뜨리던 문제
- 문서 내부 앵커 1개(기존에 깨져 있던 것)

### 작업 중 넣었다가 잡은 버그 (정직하게 기록)

Phase 5-4 에서 `Transaction.tags` 를 튜플로 바꿨는데, `parse_tags` 가 "리스트가
아니면 문자열" 로 떨어뜨리고 있어 **태그 없이 만든 거래의 tags 가 `("()",)` 가
됐다.** 오류가 나지 않아 그 시점의 테스트 130개가 전부 통과했다 — 기본 생성
경로의 tags 를 보는 테스트가 없었기 때문이다. Phase 6 에서 발견해 고치고, 입력
모양 5가지를 검증하는 회귀 테스트를 추가했다.
