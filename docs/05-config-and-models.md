...
프로그램의 **어휘**를 정의하는 다섯 모듈을 정독합니다. 값은 무엇이고, 문구는 무엇이며, 규칙은 어디에 있고, 데이터는 어떤 모양이며, 실패는 어떤 타입인가.
...
의존은 아래에서 위로 **한 방향**입니다. `errors.py` 와 루트 `config.py` 는 아무것도 import 하지 않고, `validators` 는 `errors` + 도메인 `config`·`messages` 를, `entities` 는 `validators` 와 `tx_id` 를 씁니다(→ [04 §1.2](./04-architecture.md)).
...
리팩터 전에는 `ValidationError` 가 `models.py`, `AppError` 가 `decorators.py` 에 흩어져 있었습니다. 그 결과 서비스 계층이 `AppError` 하나를 쓰려고 `decorators` 를 import 하고, `decorators` 는 다시 출력 모듈을 import 하는 **역류**가 생겼습니다.
...
`TYPE_INCOME` / `TYPE_EXPENSE` 는 리팩터에서 추가됐습니다. 이전에는 `if tx.type == "income":` 처럼 **문자열 리터럴이 로직에 박혀** 있었습니다. 상수로 뽑으면 오타가 즉시 `AttributeError` 로 드러납니다. `VALID_TYPES` 가 리터럴을 다시 적지 않고 **위 둘에서 파생**되는 것도 같은 이유입니다.
...
ID 관련 상수가 **셋**인 것이 리팩터의 흔적입니다. 셋을 쓰는 코드는 전부 값 객체 `TransactionId` 안에 있습니다([§4.4](#44-transactionid--값-객체로-모은-거래-id)).
...
`ON_DUPLICATE_CHOICES` 는 argparse 의 `choices=` 에 그대로 넘어갑니다(budget_app/cli/parser.py:226-231). **선택지 목록과 상수가 같은 자리에 있으므로** 정책을 추가할 때 한 곳만 고치면 됩니다.
...
**종료 코드** — 셸에게 무엇을 말할지는 CLI 의 계약입니다.
...
검증기가 쓰는 정규식은 **모듈 최상위에서 한 번** 컴파일합니다.
...
**입력 형태가 셋**입니다 — CSV·대화형에서 오는 **쉼표 문자열**, JSON 에서 읽은 **리스트**, 그리고 엔티티가 이미 정규화해 둔 **튜플**(`Transaction.tags`). `isinstance` 로 갈라 한 함수가 셋 다 받습니다. 이 덕분에 호출부(엔티티의 `__post_init__`)는 어디서 온 값인지 몰라도 됩니다.
...
docstring 이 말하는 분할이 실제로 어떻게 끝났는지 — 옛 `domain/models.py` 한 파일에 있던 것들이 지금 어디 있는지 표로 보면 이렇습니다. **이 절은 `entities.py` 를 중심에 두되, 함께 갈라져 나간 세 파일도 같이 봅니다.**
...
거래 id 값 객체 `TransactionId`(`domain/tx_id.py:51-127`)까지 더하면 옛 `models.py` 의 조각은 다섯 파일에 나뉘어 있습니다. 나누는 기준은 **생명주기**였습니다 — 파일에 기록되는 것(`entities`), 질의 한 번 동안만 사는 것(`queries`), 계산 결과로 만들어져 화면까지만 가는 것(`results`), 그리고 순수 함수(`periods`).
...
**달력 계산은 화면 처리가 아니라 도메인 규칙**입니다. 리팩터 전에는 이 함수가 `cli.py` 에 `_month_bounds` 라는 이름으로 있었고, 서비스는 그것을 쓸 수 없어서 문자열 접두 비교로 따로 구현했습니다. 도메인으로 내리자 두 경로가 하나로 합쳐졌습니다.
