...
> `_dispatch = handle_errors(_dispatch)` 를 실행합니다. 즉 데코레이터는 **문법 설탕**이고,
...
3. `logger.debug(템플릿, 인자)` — %-지연 포맷팅. DEBUG 가 꺼져 있으면 문자열을 아예 만들지 않습니다.
...
여기서 `d.__name__` 이 `'wrapper'` 가 아니라 `'delete'` 인 것이 요점입니다. 다만 `log_call` 의 로그 문구는 `wrapper.__name__` 이 아니라 **클로저에 잡힌 `func.__name__`** 을 쓰므로, `wraps` 가 없어도 로그 자체는 `call delete` 로 정확히 찍힙니다. `wraps` 가 실제로 구해 주는 것은 `help()`, 트레이스백, `inspect`, 그리고 "이 함수 이름이 뭐냐"고 묻는 다른 도구들입니다.
...
**`time.perf_counter()` 를 쓰는 이유**는 `time.time()` 과 달리 **단조 증가가 보장**되기 때문입니다. NTP 동기화로 시스템 시각이 뒤로 가면 `time.time()` 차이는 음수가 될 수 있습니다.
...
왜 여기서 처리하지 않는가 — `list | head` 에서 `head` 가 먼저 닫히면 stdout 이 깨진 상태입니다. 이 상황에서 `output.err(...)` 를 부르면 `err()` 안의 `sys.stdout.flush()` 가 또 `BrokenPipeError` 를 냅니다(그래서 `output.err` 도 그 예외를 삼키도록 되어 있습니다 — cli/output.py:60-65). 근본 해결은 **stdout 을 통째로 블랙홀로 갈아끼우는 것**이고, 그 일은 `main()` 이 합니다.
...
> 갈라야 했습니다. PEP 3151 이 이 `errno` 검사를 **예외 계층으로 승격**시켜,
...
**`OSError` 포괄 절**은 디스크 가득 참(ENOSPC), 파일 잠금 같은 나머지를 받습니다. 원자적 쓰기 중 디스크가 차면 여기로 옵니다 — 그리고 임시 파일에 쓰던 중이므로 **원본은 무사합니다**.
...
**저장소는 "없었다"는 사실만 보고**(`False`), **서비스가 그것을 "오류"로 판정**합니다(`AppError`). 왜 이렇게 나누는가 — "id 가 없다"가 항상 오류인 것은 아니기 때문입니다. 예를 들어 "있으면 지우고 없으면 넘어가는" 멱등 삭제를 나중에 추가한다면, 저장소는 그대로 두고 서비스만 바꾸면 됩니다.
