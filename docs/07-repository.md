...
가계부 프로그램이 저장한 내용은 결국 컴퓨터 안의 파일 몇 개입니다. 이 문서는 그 파일을 읽고 쓰는 부분만 따로 모아 둔 자리를 설명합니다. 파일에 무언가를 적는 일은 생각보다 까다롭습니다. 적는 도중에 정전이 되거나 프로그램이 강제로 종료되면 파일이 반쯤 적힌 채로 남을 수 있습니다. 그러면 적다 만 그 줄 하나를 잃고, 나중에 그 뒤에 이어 쓴 줄까지 함께 못 읽게 될 수 있습니다 — 방금 저장했다고 알린 기록이 그렇게 사라집니다. 다행히 앞에 적어 둔 줄들은 그대로 읽힙니다. 사람이 파일을 직접 열어 잘못 고쳐 놓는 일도 있습니다. 그래서 이 부분의 코드는 대부분 잘될 때가 아니라 **잘못될 때**를 위한 장치입니다. 무엇을 막을 수 있고 무엇은 끝내 막지 못하는지까지 적어 두는 것이 이 문서의 목표입니다.

**이 문서에 자주 나오는 말**

| 말 | 쉬운 뜻 |
| --- | --- |
| JSONL | 한 줄에 한 건씩 적어 내려가는 장부 파일. 줄 하나가 기록 한 건입니다 — 거래·카테고리·예산이 각각 이 형식으로 자기 파일에 쌓입니다 |
| 원자적 교체 | 파일 **하나**를 바꿀 때, 바꾸다 만 중간 상태가 남지 않게 통째로 갈아 끼우는 것. 파일 둘 이상을 한꺼번에 바꾸는 일까지는 이것으로도 막지 못합니다(§9.3) |
| 스트리밍(제너레이터) | 파일을 통째로 들고 오지 않고 필요할 때 한 줄씩 꺼내 오는 방식 |
| 손상된 줄 | 프로그램이 해석하지 못하는 줄. 사람이 잘못 고쳤거나 저장이 중간에 끊겼을 때 생깁니다 |
| fsync | 적은 내용이 실제 저장 장치까지 내려갔는지 확인받는 것. 하지 않으면 정전 때 사라질 수 있습니다 |
| 워터마크 | 지금까지 발급한 번호 중 가장 큰 것을 따로 적어 둔 기록. 줄어들지 않습니다 |
| 저장소(리포지토리) | 파일 하나를 맡아 읽고 쓰는 담당자. 무엇이 옳은 값인지는 판단하지 않습니다 |
| 커밋 | 준비해 둔 변경을 실제로 반영하는 마지막 한 걸음 |

**바쁘면 여기만**

- **§2** — 실제로 났던 사고 두 건이 터미널 화면 그대로 나옵니다. 이 문서의 나머지가 전부 이 두 건의 뒷이야기입니다.
- **§3.6** — "쓰다가 컴퓨터가 꺼져도 안전하다"가 정확히 어떤 순서로 만들어지는지가 여기 있습니다.
- **§9.3** — 이 방식이 무엇을 **못** 막는지 적어 둔 표입니다. 한계를 알아야 보장의 크기도 알 수 있습니다.

---

파일을 읽고 쓰는 모든 코드가 모여 있는 계층입니다. 프로그램이 파일을 쓰다가 컴퓨터가 꺼지면 어떻게 되는가 — 이 계층은 그 질문에 답하려고 있습니다.

다루는 것은 다섯 가지입니다.

- **스트리밍 읽기** — 파일을 통째로 메모리에 올리지 않고 한 줄씩 꺼내 보는 것
- **원자적 교체** — 바꾸다 만 중간 상태가 남지 않게 통째로 한 번에 갈아 끼우는 것
- **손상된 줄 보존** — 해석하지 못한 줄도 원문 그대로 파일에 남겨 두는 것
- **ID 발급 충돌 방지** — 같은 번호가 서로 다른 두 거래에 붙는 일을 막는 것
- **CSV 경계 어댑터** — 바깥과 주고받는 표 형식 파일을 우리 데이터로 옮겨 주는 얇은 번역층
...
이 절이 보는 것은 "이 폴더가 무엇을 하기로 약속했는가"입니다. 약속은 두 줄로 줄어듭니다. 파일을 여는 코드는 이 폴더 밖에 두지 않는다, 그리고 이 폴더 안에서는 무엇이 옳은 값인지 판단하지 않는다. 판단은 도메인과 서비스가 하고, 이 계층은 완성된 것을 받아 적고 다시 꺼내 줄 뿐입니다.
...
> **💡 쉽게 말하면** — 이 프로그램이 쓰는 저장 형식(JSONL)은 공책 장부와 같습니다. 한 줄에 기록 한 건을 적고, 새 기록이 생기면 그다음 줄에 이어 적습니다. 파일 전체를 읽지 않아도 줄 하나만 보면 그 한 건을 알 수 있고, 사람이 열어 봐도 그냥 읽힙니다. 거래도 카테고리도 예산도 각각 이 형식으로 자기 파일에 쌓입니다.
> 다만 이 비유는 "고쳐 쓰기"에서 깨집니다 — 공책은 한 줄만 지우고 그 자리에 다시 쓸 수 있지만, 파일은 가운데를 고치는 순간 원본이 반쯤 고쳐진 상태로 남습니다. 그래서 이 코드는 일부러 옆에 통째로 새로 쓴 뒤 마지막에 갈아 끼웁니다(§3.5·§3.6) — 물리적으로 못 해서가 아니라 원본을 끝까지 건드리지 않으려는 선택입니다. 물리적으로 정말 안 되는 것은 따로 있습니다. 가운데 줄 하나를 빼내 파일을 짧게 만드는 일은 애초에 되지 않습니다.
...
프로그램이 이 파일을 읽는 방법은 두 가지입니다. 하나면 될 것 같은데 왜 둘인가 — 이 절의 주제가 그것입니다. 답은 간단합니다. 하나로 두었더니 사고가 두 건 났기 때문입니다. 리팩터 이전과 이후를 먼저 대조합니다. 이 하나가 이 문서 내용의 절반입니다.
...
> **💡 쉽게 말하면** — 글씨가 번져 못 읽는 줄이 섞인 장부를 새 장부로 옮겨 적는 상황입니다. 못 읽겠다고 그 줄을 빼 버리면, 나중에는 그런 줄이 있었다는 사실조차 아무도 모릅니다. 못 읽는 모양 그대로 베껴 두면 원래 무엇이 적혀 있었는지 나중에 사람이 확인할 수 있습니다. `RawLine` 은 "원문"과 "해석 결과"를 한 상자에 같이 담아 그 베껴 적기를 가능하게 합니다.
> 다만 이 비유는 "짐작"에서 깨집니다 — 사람은 번진 글씨에서 뜻을 어림잡지만 프로그램은 어림잡지 않습니다. 해석에 실패한 줄은 원문만 보존될 뿐 조회 결과에는 끼지 못하고, 경고 로그로만 알려집니다(§3.4).
...
| JSON 도 아님 | 있음 | None | None | **원문 보존** | 정규식(글자 모양 패턴)으로 발굴 |

---

## 3. `JsonlStore` — 파일 포맷을 다루는 법 (제네릭 기반 클래스)

저장되는 데이터는 세 종류입니다. 거래, 카테고리, 예산이고 각각 파일 하나씩을 씁니다. 그런데 파일을 열고, 한 줄씩 읽고, 안전하게 다시 쓰는 방법은 셋 다 똑같습니다. 그 똑같은 부분만 부모 클래스 하나로 올린 것이 `JsonlStore` 이고, 여기에 제네릭(어떤 종류의 데이터를 담을지 나중에 정하게 열어 둔 틀)이 쓰입니다.
...
`is_empty` 의 `not self.path.exists() or ...` 순서에 주의하세요. 파일이 없으면 `stat()` 이 예외를 내므로 **단락 평가(앞의 조건만으로 답이 정해지면 뒤는 아예 실행하지 않는 규칙)로 앞에서 막습니다.**
...
이 메서드는 파일의 모든 줄을 하나씩 내보냅니다. 해석에 실패한 줄도 버리지 않고, 원문을 그대로 실어 함께 내보냅니다.
...
> **💡 쉽게 말하면** — 파일에 담긴 것은 사실 글자가 아니라 바이트이고, 그것을 약속된 규칙(여기서는 UTF-8)에 따라 글자로 옮겨 읽습니다. 규칙에 맞지 않는 자국을 만났을 때 택할 수 있는 길은 셋입니다. 옮기기를 포기하고 문서를 통째로 덮거나, 물음표로 뭉개 버리거나, "여기 있던 자국은 몇 번"이라고 아무도 쓰지 않는 임시 번호를 적어 두거나. 세 번째만이 나중에 원래 자국을 한 치도 다르지 않게 되돌려 놓을 수 있고, `surrogateescape` 가 그 세 번째입니다.
> 다만 이 비유는 "임시 번호를 누가 알아보는가"에서 깨집니다 — 되돌려 적는 쪽이 같은 규칙을 쓸 때만 성립합니다. 읽기와 쓰기 중 한쪽만 이 정책이면 보존한 줄이 뭉개지거나 쓰기가 아예 실패합니다(아래 표).
...
파일을 고칠 때 원본에 바로 덮어쓰지 않습니다. 옆에 임시 파일을 하나 만들어 새 내용을 전부 적고, 그것이 디스크에 확실히 내려간 뒤에, 마지막에 이름만 갈아 끼웁니다. 이 절이 그 세 단계를 나눠 보는 자리입니다.

> **💡 쉽게 말하면** — 벽에 붙은 안내문을 고치는 일과 같습니다. 붙어 있는 종이 위에 지우고 덧쓰면, 고치는 동안 지나가는 사람은 지우다 만 문장을 봅니다. 새 종이에 다 쓴 다음 압정을 뽑아 한 번에 갈아 끼우면, 누가 언제 보든 옛 안내문 아니면 새 안내문 하나만 봅니다. 코드가 하는 일이 정확히 뒤쪽입니다.
> 다만 이 비유는 "같은 벽"이라는 조건에서 깨집니다 — 이름을 갈아 끼우는 `os.replace` 가 쪼개지지 않는 것은 같은 파일시스템(같은 디스크 구획) 안에서일 뿐이고, 임시 파일을 다른 곳에 만들면 성립하지 않습니다. 아래 `with_suffix` 노트가 임시 파일을 굳이 같은 폴더에 만드는 이유입니다.
...
> **💡 쉽게 말하면** — 편지에 빗대면 세 단계가 이렇습니다. 편지를 다 썼다(`f.write`), 봉투에 넣어 현관에 내놨다(`f.flush`), 우체통에 실제로 넣었다(`os.fsync`). 현관까지만 가 있는 편지는 그 집에 불이 나면 함께 탑니다 — 전원이 갑자기 끊기면 아직 OS 버퍼에 있던 내용이 그렇게 됩니다.
> 다만 이 비유는 "몇 통인가"에서 깨집니다 — `fsync` 는 편지 한 통이 아니라 그 파일에 대해 아직 내려가지 않은 것을 한꺼번에 내립니다. 그리고 이 셋은 전부 **내용**에 대한 이야기라, "이름이 바뀌었는가"는 여전히 별개입니다. 그 몫이 위 표 세 번째 줄의 `os.replace` 입니다.
...
거래 하나하나에는 `TX-000001` 같은 번호가 붙습니다. 그 번호를 누가 어떤 규칙으로 정하는지가 이 절의 주제입니다. 규칙이 한 곳에만 있어야 하는 이유, 그리고 한 번 지운 번호를 다시 쓰면 왜 위험한지를 다룹니다.
...
`IdWatermark`(`storage/ids.py:26-78`)는 **발급된 적 있는 최대 번호를 숫자 한 줄짜리 파일(`id_counter`)에 남깁니다.** 파일 스캔 최대값은 "지금 무엇이 있는가"라 삭제하면 줄어들고, 그러면 지운 번호가 부활해 예전에 내보낸 CSV 를 다시 가져올 때 서로 다른 거래가 "이미 있는 id"로 판정돼 조용히 버려집니다. 워터마크(발급한 적 있는 최대 번호를 따로 적어 둔, 줄어들지 않는 기준선)는 그 경로를 막습니다. 이 파일이 없거나 깨져도 `read()` 가 0 을 돌려주므로 **안전장치가 고장 나도 본체는 멈추지 않습니다.**

> **💡 쉽게 말하면** — 은행 번호표 발급기와 같습니다. 번호표를 뽑은 손님이 그냥 돌아가도 기계는 눈금을 되돌리지 않습니다. 되돌렸다가 다음 손님에게 같은 번호를 주면, 아까 그 번호로 접수된 서류와 헷갈리기 때문입니다. `id_counter` 파일이 그 되돌리지 않는 눈금이고, 지금 파일에 남아 있는 거래 중 **가장 큰 번호**를 보고 그다음을 주는 방식은 손님이 나갈 때마다 눈금을 되돌리는 것과 같습니다.
> 다만 이 비유는 "고장 났을 때"에서 깨집니다 — 번호표 기계가 고장 나면 창구가 멈추지만, 이 파일은 없거나 깨져도 `read()` 가 0 을 돌려주고 프로그램은 그대로 돕니다. 그동안은 파일 스캔 값만 남으므로 보호가 없던 시절로 돌아갈 뿐입니다.
...
거래 파일 하나를 맡는 담당자입니다. `JsonlStore` 를 상속(부모 클래스가 가진 것을 그대로 물려받는 것)하고 거래에만 있는 것을 더합니다. 거래에만 있는 **규칙**은 둘입니다 — ID 발급, 그리고 카테고리 일괄 재지정. 여기에 거래 전용 조회·수정 연산(`get` · `exists` · `delete` · `replace`)이 함께 붙습니다(§5.1).
...
카테고리 파일에 들어 있는 것은 이름 목록입니다. 여기서 신경 쓸 일은 둘뿐입니다. 같은 이름이 두 번 들어가지 않게 하는 것, 그리고 파일이 비어 있는 첫 실행 때 기본 이름 몇 개를 넣어 주는 것(시딩)입니다.
...
예산은 한 달에 하나만 있어야 합니다. 그런데 저장 형식은 줄을 이어 적는 장부라 그냥 두면 같은 달이 여러 줄 쌓입니다. 그래서 같은 달 예산을 다시 정할 때는 옛 줄을 지우고 새 줄을 붙이는 일을 **한 번에** 합니다.
...
CSV 는 엑셀 같은 다른 프로그램과 주고받는 표 형식 파일입니다. 이 프로그램이 평소에 쓰는 형식과 다르니 번역이 필요합니다. 그 번역만 맡는 얇은 층이 `csv_io.py` 이고, "경계 어댑터"라는 이름이 그 자리를 가리킵니다. 번역만 하고, 무엇을 받아들이고 무엇을 버릴지는 정하지 않습니다.
...
**BOM 방침이 docstring 에 명시된 이유**는 실제로 겪을 수 있는 함정이기 때문입니다. BOM 은 파일 맨 앞에 붙는, 화면에는 보이지 않는 표식입니다. Excel 호환을 위해 `utf-8-sig` 로 바꾸면 헤더 첫 컬럼명이 `﻿id` 가 되어 자기 파일을 다시 읽지 못합니다.
...
백업은 데이터 폴더의 파일들을 시각이 찍힌 새 폴더로 복사하는 일입니다. 내용을 해석하지 않고 바이트를 그대로 옮기므로 도메인 판단이 한 줄도 없습니다. 그래서 서비스가 아니라 이 계층에 있습니다.
...
여기까지는 파일 하나를 안전하게 바꾸는 이야기였습니다. 이 절은 파일 **둘**을 함께 바꿔야 할 때의 이야기입니다. 하나는 바뀌고 하나는 안 바뀐 상태를 어떻게 줄이는가 — 그리고 그 상태를 완전히 없애지는 못한다는 사실을 어떻게 정직하게 적는가입니다. 준비(임시 파일 작성)와 커밋(실제 반영), 롤백(준비한 것을 되돌리기)이라는 세 낱말이 이 절의 뼈대입니다.

> **💡 쉽게 말하면** — 서류 두 장을 함께 갈아 끼워야 하는 상황입니다. 두 장을 각각 새로 써 두었다가, 다 준비된 뒤에 마지막에 연달아 갈아 끼웁니다. 시간이 오래 걸리는 것은 새로 쓰는 쪽이고 그동안 원본 두 장은 그대로이므로, 도중에 그만두면 새로 쓰던 종이만 버리면 됩니다.
> 다만 이 비유는 "연달아"와 "동시에"의 차이에서 깨집니다 — 첫 장을 갈아 끼운 직후 전원이 끊기면 한쪽만 바뀐 상태가 남습니다. 이 방식이 하는 일은 그 틈을 없애는 것이 아니라 밀리초 단위로 줄이는 것이고, §9.3 표의 마지막 줄이 그 한계입니다.
...
구술로 받을 법한 질문과 답을 모았습니다. 앞의 절에서 다룬 것을 한 문단씩으로 줄인 것이라, 시간이 없으면 여기부터 읽고 궁금한 절로 되짚어 가도 됩니다.
...
`JsonlStore(Generic[T])` 에 "JSONL 파일을 다루는 법"(열기·스트리밍·원자적 재작성·추가)을 두고, 하위 클래스는 엔티티 고유의 것만 갖습니다 — `TransactionRepository` 는 ID 발급과 카테고리 재지정, `CategoryStore` 는 이름 중복과 기본값 시딩, `BudgetStore` 는 "같은 달은 덮어쓰기". 한 줄을 세우는 것은 `self.entity_cls.from_dict(data)` 인데, 세 dataclass 가 같은 이름의 classmethod 를 갖고 있어서 가능한 덕 타이핑(무슨 타입이라고 선언했는지가 아니라 필요한 것을 실제로 갖고 있는지로 판정하는 방식)입니다.
