...
budget_app 은 **파일 기반 가계부 콘솔 프로그램**입니다. 터미널에 명령을 입력해 수입/지출 거래를 기록합니다 — 거래 추가는 프로그램이 날짜·금액 같은 항목을 하나씩 물어보는 대화형이라 명령 한 줄로 끝나지 않습니다(§4.1). 그 밖에 목록 보기, 조건 검색, 월별 요약, 예산 관리, CSV 가져오기/내보내기를 할 수 있습니다. 데이터베이스 서버는 쓰지 않습니다. 모든 데이터는 이 컴퓨터 안의 JSONL(JSON Lines — 한 줄에 기록 한 건씩 적는 텍스트 형식) 파일 3개에 저장됩니다.
...
> **조건값도 저장값과 같은 규칙으로 정규화(표기를 한 가지 모양으로 다듬는 일 — 여기서는 앞뒤 공백 제거)됩니다.** `--category " food "` 처럼 공백이 낀 입력은 저장 시점과 똑같은 함수로 다듬어집니다. 다만 그 일을 하는 자리가 `SearchFilter` 가 아니라는 점이 중요합니다. `SearchFilter.__post_init__`(`domain/queries.py:54-55`)은 조건들을 명세 트리로 **조립만** 하고, 실제 정규화는 각 명세의 생성자가 합니다 — `specs.InCategory.__init__`(`domain/specs.py:199-200`)이 `validators.parse_category` 를 직접 부릅니다. 규칙이 한 곳(`domain/validators.py`)에만 있어야 "저장할 때의 규칙"과 "찾을 때의 규칙"이 갈라지지 않기 때문입니다. 갈라지면 검색은 오류 없이 조용히 틀립니다.
...
세 파일 모두 **JSONL(JSON Lines)** 형식입니다. 규칙은 "한 줄 = JSON 객체 1개 = 레코드 1건" 하나뿐입니다. 그래서 특별한 도구 없이 텍스트 에디터로 열어 바로 읽을 수 있습니다.
