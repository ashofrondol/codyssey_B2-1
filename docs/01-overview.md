...
budget_app 은 돈이 들어오고 나간 일을 적어 두는 가계부입니다. 다만 창과 버튼을 눌러 쓰는 프로그램이 아니라, 화면에 글자를 쳐서 시키고 글자로 답을 받는 방식입니다. 적은 내용은 인터넷 어딘가가 아니라 이 컴퓨터의 폴더 안, 텍스트 파일 세 개에 그대로 쌓입니다(프로그램이 스스로 쓰는 번호표 파일 `id_counter` 까지 하면 네 개입니다). 그래서 메모장으로 열어 봐도 무엇이 적혀 있는지 눈으로 읽힙니다. 이 문서는 그 프로그램을 어떻게 켜고, 무엇을 시킬 수 있고(11가지), 적은 것이 어디에 어떤 모양으로 남는지를 훑습니다. 프로그램 속이 어떻게 만들어졌는지는 뒤의 문서들이 맡고, 여기서는 **쓰는 사람의 자리**에서만 봅니다.

**이 문서에 자주 나오는 말**

| 말 | 쉬운 뜻 |
| --- | --- |
| 콘솔(터미널) | 그림도 버튼도 없이 글자만 주고받는 화면. 한 줄 쳐서 시키면 결과가 글자로 돌아옵니다 |
| 명령 | 프로그램에 시키는 일 한 가지. `add`(추가), `list`(목록)처럼 이름이 붙어 있습니다 |
| 옵션 | 명령 뒤에 붙여 세부 사항을 정하는 말. `--limit 3` 은 "3건만 보여 줘"라는 뜻입니다 |
| 표준 라이브러리 | 파이썬을 설치하면 함께 딸려 오는 기본 도구 모음. 이 프로그램은 여기 있는 것만 씁니다 |
| 모듈 · 패키지 | 파이썬 파일 하나가 모듈, 그 파일들을 담은 폴더가 패키지입니다 |
| JSONL | 한 줄에 기록 한 건씩 적어 둔 텍스트 파일 형식 |
| 종료 코드 | 프로그램이 끝나며 남기는 숫자 한 개. 0 은 "잘 끝났다", 그 밖의 숫자는 왜 그렇게 끝났는지를 가리키는 번호입니다 — 오류의 종류일 수도 있고 사용자가 Ctrl+C 로 스스로 멈춘 것일 수도 있습니다(§6) |
| 대화형 | 프로그램이 한 항목씩 물어보고 사람이 답을 쳐 넣는 방식 |

**바쁘면 여기만**

- **§1 이 프로젝트는 무엇인가** — 이게 대체 무엇을 하는 물건인지가 한 화면에 들어 있습니다.
- **§4 명령어 11종 상세** — 실제로 무엇을 시킬 수 있고 화면에 어떻게 찍히는지를 예시로 봅니다. 여기까지 읽으면 프로그램을 쓸 수 있습니다.
- **§6 종료 코드 표** — 뭔가 잘못됐을 때 화면에 남은 숫자가 무슨 뜻인지 찾아보는 표입니다.
...
budget_app 은 **파일 기반 가계부 콘솔 프로그램**입니다. 터미널에 명령을 입력해 수입/지출 거래를 기록합니다 — 거래 추가는 프로그램이 날짜·금액 같은 항목을 하나씩 물어보는 대화형이라 명령 한 줄로 끝나지 않습니다(§4.1). 그 밖에 목록 보기, 조건 검색, 월별 요약, 예산 관리, CSV 가져오기/내보내기를 할 수 있습니다. 데이터베이스 서버는 쓰지 않습니다. 거래·카테고리·예산은 이 컴퓨터 안의 JSONL(JSON Lines — 한 줄에 기록 한 건씩 적는 텍스트 형식) 파일 3개에 저장되고, 그와 별도로 발급한 거래 번호를 기억하는 파일 1개(`id_counter`)가 같은 폴더에 놓입니다.

> **💡 쉽게 말하면** — 창과 버튼이 있는 프로그램이 점원에게 손짓으로 주문하는 것이라면, 콘솔 프로그램은 주문 내용을 종이에 적어 창구로 밀어 넣는 것입니다. 손짓보다 배울 것이 많은 대신, 적어 둔 주문서는 그대로 다시 쓸 수도 있고 남에게 건넬 수도 있습니다 — 명령을 파일에 모아 두면 자동 실행이 되는 것이 그래서입니다(CSV 를 통째로 들여오는 `import` 가 그런 쪽이고, 되묻는 `add` 는 답까지 미리 넣어 주어야 합니다).
> 다만 이 비유는 "알아서 이해해 주는" 부분에서 깨집니다 — 사람 점원과 달리 콘솔은 글자 하나만 틀려도 알아듣지 못하고, 형식이 정확히 맞아야만 받아 줍니다.

세 가지 배경을 먼저 알아두면 좋습니다.

1. **표준 라이브러리(파이썬을 설치하면 함께 딸려 오는 기본 도구 모음)만 사용합니다.** `pyproject.toml` 의 `dependencies = []` 가 보여주듯 외부 패키지 의존성이 0입니다. `argparse`(명령 파싱), `json`(저장 형식), `csv`(가져오기/내보내기), `dataclasses`(모델), `pathlib`(경로), `logging`(로그) 등 파이썬에 기본 내장된 모듈만 씁니다. 따라서 Python 3.10 이상만 있으면 `pip install` 없이 바로 실행됩니다.
...
폴더 이름부터 보면 나머지가 따라옵니다. `budget_app/` 아래에는 폴더가 넷 있고, 그 넷이 곧 이 프로그램의 **계층**(일을 성격별로 갈라 놓은 층)입니다. 아직 파일 이름을 외울 필요는 없습니다. "이런 갈래로 나뉘어 있구나" 정도만 보고 넘어가도 됩니다.

> **💡 쉽게 말하면** — 네 폴더는 식당의 홀·주방·창고·메뉴판과 같습니다. 홀 직원(`cli/`)은 손님의 주문을 받아 주방에만 넘기고, 요리사(`services/`)는 무엇을 어떻게 낼지 판단하며, 창고 담당(`storage/`)만 재료가 실제로 놓인 선반에 손을 댑니다. 무엇이 요리이고 무엇이 재료인지에 대한 약속(`domain/`)은 셋이 함께 지킵니다. 홀 직원이 창고에 직접 들어가지 않으니, 창고 정리 방식을 바꿔도 홀은 그대로입니다.
> 다만 이 비유는 "급하면 그냥 들어가면 되지"에서 깨집니다 — 실제 식당과 달리 여기서는 그 지름길이 아예 막혀 있고, 어기면 테스트가 깨집니다(§7.4).
...
│   ├── budgets.jsonl          # 월별 예산
│   └── id_counter             # 발급한 최대 거래 번호 (JSONL 아님)
...
| `__main__.py` | `python -m budget_app` 으로 실행될 때의 진입점(프로그램이 맨 처음 시작되는 자리)입니다. |
...
| `decorators.py` | `@log_call`, `@measure_time` — 호출 기록과 실행 시간 측정, 즉 관측만 담당하는 **데코레이터**(원래 함수는 그대로 두고 겉을 감싸 할 일을 덧붙이는 표기)입니다. |
| `context.py` | 합성 루트(부품을 만들어 서로 끼우는 일을 한곳에서만 하는 자리). 저장소 3개 + 서비스 5개를 조립하는 `AppContext` 하나만 있습니다(§3.5). |
...
| `storage/jsonl.py` | JSONL 을 한 줄씩 읽고(제너레이터 — 파일을 통째로 메모리에 올리지 않고 필요할 때 한 줄씩 꺼내 주는 방식), 임시 파일 + `os.replace` 로 안전하게 씁니다. 이 패키지에서 가장 큰 파일(329줄)입니다. |
...
일반 개념부터 설명하면, `python -m 패키지명` 은 "그 패키지를 **모듈로서** 실행하라"는 뜻입니다. 파이썬 인터프리터(파이썬 코드를 읽어 실제로 실행해 주는 프로그램)는 `budget_app` 패키지를 찾은 뒤, 그 안의 `__main__.py` 파일을 `__name__ == "__main__"` 상태로 실행합니다. 즉 `__main__.py` 는 패키지의 "실행 버튼" 역할을 하는 관례적 파일명입니다.

> **💡 쉽게 말하면** — 이 명령은 파일 경로를 손가락으로 짚어 주는 것이 아니라 **이름을 부르는 것**입니다. "파이썬아, `budget_app` 이라는 이름의 꾸러미를 네 방식대로 찾아서, 그 안의 실행 버튼을 눌러라"에 가깝습니다. 꾸러미 안에서 어느 파일이 실행 버튼인지는 미리 약속돼 있고, 그 약속된 이름이 `__main__.py` 입니다.
> 다만 이 비유는 "부르면 어디서든 온다"에서 깨집니다 — 파이썬이 찾아보는 자리에 그 꾸러미가 있어야 하고, 그래서 프로젝트 루트에서 실행해야 합니다.
...
`sys.exit(main())` 은 `main()` 이 반환한 정수를 그대로 **프로세스 종료 코드**(프로그램이 끝나면서 바깥에 남기는 숫자 한 개 — §6)로 만듭니다. `main()` 은 `cli/app.py` 에 있습니다.
...
> **조건값도 저장값과 같은 규칙으로 정규화(표기를 한 가지 모양으로 다듬는 일 — 여기서는 앞뒤 공백 제거)됩니다.** `--category " food "` 처럼 공백이 낀 입력은 저장 시점과 똑같은 함수로 다듬어집니다. 다만 그 일을 하는 자리가 `SearchFilter` 가 아니라는 점이 중요합니다. `SearchFilter.__post_init__`(`domain/queries.py:55-56`)은 조건들을 명세 트리로 **조립만** 하고, 실제 정규화는 각 명세의 생성자가 합니다 — `specs.InCategory.__init__`(`domain/specs.py:199-200`)이 `validators.parse_category` 를 직접 부릅니다. 규칙이 한 곳(`domain/validators.py`)에만 있어야 "저장할 때의 규칙"과 "찾을 때의 규칙"이 갈라지지 않기 때문입니다. 갈라지면 검색은 오류 없이 조용히 틀립니다.

### 4.4 summary — 월별 요약

지정한 달의 총 수입·총 지출·잔액을 계산해 보여줍니다. 그 달의 예산이 설정되어 있으면 사용률과 초과 경고를 덧붙입니다. 지출이 많은 카테고리 TOP N 도 함께 나옵니다.
...
> **💡 쉽게 말하면** — 장바구니를 계산대에 올렸는데 물건 하나에 바코드가 없는 상황입니다. `--atomic` 없이는 그 물건만 빼고 나머지를 계산하고, `--atomic` 을 붙이면 장바구니를 통째로 물립니다. 어느 쪽이 옳은지는 상황에 달렸습니다 — 100건 중 3건이 문제라면 97건을 살리는 편이 낫고, 서로 아귀가 맞아야 하는 결산 자료라면 통째로 물리는 편이 안전합니다.
> 다만 이 비유는 "이미 산 물건을 또 올린 경우"에서 깨집니다 — 그건 바코드가 없는 것과 전혀 다른 문제라서 `--on-duplicate` 라는 별도의 축으로 갈라져 있습니다.
...
`--month 2024-01` 을 주면 `SearchFilter.for_month`(`domain/queries.py:75-79`)가 `domain/periods.py:20-30` 의 `month_range` 를 통해 `2024-01-01` ~ `2024-01-31`(그 달의 실제 말일)로 변환합니다. 2월이나 30일까지인 달도 `calendar.monthrange` 로 정확한 말일을 계산합니다. **`summary` 도 같은 함수를 씁니다** — "이 달에 속하는가"의 정의가 프로그램 전체에서 하나입니다.
...
## 5. 데이터 파일(JSONL 3종 + id_counter)과 기본 카테고리 시드

### 5.1 JSONL 형식

거래·카테고리·예산 세 파일 모두 **JSONL(JSON Lines)** 형식입니다(같은 폴더의 `id_counter` 만은 JSONL 이 아닙니다 — 아래에서 따로 다룹니다). 규칙은 "한 줄 = JSON 객체 1개 = 레코드 1건" 하나뿐입니다. 그래서 특별한 도구 없이 텍스트 에디터로 열어 바로 읽을 수 있습니다.
...
콘솔 프로그램은 종료 코드(exit code)로 성공/실패의 종류를 호출자(셸, 스크립트, 채점기)에게 알립니다. 이 프로젝트는 8종을 `cli/config.py` 에 상수로 정의합니다. 이 값이 도메인이나 저장소가 아니라 **CLI 계층**의 config 에 있는 데는 이유가 있습니다. "셸에게 무엇을 말할 것인가"는 CLI 가 바깥과 맺은 약속이기 때문입니다. 서비스는 `AppError` 를 던질 뿐, 그것이 4번으로 번역된다는 사실을 모릅니다.

> **💡 쉽게 말하면** — 심부름을 보낸 뒤 돌아온 사람에게 듣는 한마디와 같습니다. 상대가 사람이면 "다녀왔어요", "가게 문이 닫혔더라고요" 라고 말하면 됩니다. 그런데 다음 차례를 기다리는 것이 사람이 아니라 다른 프로그램일 때는 문장을 읽어 낼 수 없으므로, 미리 번호를 정해 둡니다 — 0 은 "됐다", 3 은 "파일을 못 찾았다" 하는 식입니다. 화면의 `[오류]` 문장은 사람 몫이고, 숫자는 기계 몫입니다.
> 다만 이 비유는 "번호가 만국 공통"에서 깨집니다 — 0 이 성공이라는 것 말고는 프로그램마다 제 나름대로 정하며, 아래 표도 이 프로그램 안에서만 통합니다(130 만은 예외로 셸의 관례입니다).
...
`pyproject.toml` 은 파이썬 프로젝트의 표준 설정 파일입니다. 이 프로젝트에서는 네 가지 역할을 합니다 — (1) 프로젝트 메타데이터(이름·버전 같은 신원 정보), (2) uv 실행 방식, (3) Ruff 린터(코드를 실행하지 않고 훑어보며 스타일 위반과 실수를 지적하는 도구)/포매터 설정, (4) pytest 설정입니다.

> **💡 쉽게 말하면** — 이 파일은 제품 상자 옆면에 붙은 사양표에 가깝습니다. 이름·버전·필요한 파이썬 버전 같은 것이 적혀 있고, 그 아래 `[tool.*]` 칸들은 이 프로젝트에 드나드는 도구들이 각자 쓰는 서랍입니다. 서랍마다 이름표가 붙어 있어서 다른 도구는 열어 보지 않습니다.
> 다만 이 비유는 "제품 자신을 설명한다"에서 깨집니다 — 정작 프로그램 본인은 이 종이를 읽지 않습니다. `python -m budget_app` 은 `pyproject.toml` 을 열지도 않고, 읽는 것은 uv·Ruff·pytest 같은 바깥 도구뿐입니다(§7.1).
