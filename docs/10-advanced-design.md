...
이 문서는 이 가계부 프로그램이 **약속할 수 있는 것과 약속할 수 없는 것**을 가려 적은 글입니다. 가계부는 결국 컴퓨터 안의 파일 몇 개에 장부를 적어 두는 일인데, 적는 도중에 전원이 나갈 수도 있고, 같은 장부를 두 사람이 동시에 고칠 수도 있고, 장부가 두꺼워질수록 뒤적이는 시간이 길어지기도 합니다. 이 문서는 그런 일이 실제로 벌어지면 무엇이 남고 무엇이 사라지는지를 하나씩 따져 봅니다.

보통 이런 글은 잘된 점만 적습니다. 이 문서는 반대로 **못 하는 것, 그리고 일부러 하지 않기로 한 것**에 같은 분량을 씁니다. 그래서 읽고 나면 "이 프로그램을 어디까지 믿어도 되는가"라는 질문에 답할 수 있게 됩니다. 그 경계를 정확히 말하는 편이 "완벽합니다"라고 말하는 것보다 낫다는 것이 이 문서의 태도입니다.

**이 문서에 자주 나오는 말**

| 말 | 쉬운 뜻 |
| --- | --- |
| 원자적 교체 | 같은 파일시스템 안에서, 바꾸다 만 중간 상태가 바깥에 보이지 않게 통째로 한 번에 갈아 끼우는 것. 그래도 남는 한계가 있고 §1.4·§1.5 에 적었습니다 |
| crash | 프로그램이 하던 일을 끝내지 못하고 갑자기 멈추는 것(전원 차단, 강제 종료 등) |
| fsync | 쓴 내용을 저장 장치까지 실제로 내려 보내라고 시키는 것. 이걸 안 하면 전원이 나갔을 때 사라질 수 있습니다 |
| 불변식 | 언제 확인해도 반드시 참이어야 하는 약속 |
| 멱등 | 같은 일을 여러 번 해도 결과가 한 번 한 것과 같은 성질 |
| 복잡도 / O(N) | 자료가 두 배로 늘면 비용도 두 배로 는다는 식으로, 규모와 비용의 관계만 적는 표기 |
| lost update | 두 사람이 각자 고쳐 놓고 나중 사람이 덮어써서, 앞사람 수정이 소리 없이 사라지는 일 |
| 잠금(lock) | 한쪽이 파일을 고치는 동안 다른 쪽이 손대지 못하게 걸어 두는 장치. 이 프로그램에는 없습니다 |

**바쁘면 여기만**

- **§1.3 crash 시나리오 4종** — 저장하는 도중에 전원이 나가면 무엇이 남는지를 네 경우로 나눠 보여 줍니다. 이 문서에서 가장 구체적인 대목입니다.
- **§6 동시성의 한계** — 같은 프로그램을 동시에 두 번 켜면 왜 위험한지. 이 설계가 끝내 막지 못하는 종류의 사고입니다.
- **§9 정리** — 문서 전체를 여섯 개의 질문과 답으로 줄여 둔 절입니다. 여기부터 읽고 궁금한 항목만 되짚어도 됩니다.
...
파일 하나를 안전하게 바꿔 쓰는 일은 서로 다른 세 가지 걱정으로 나뉩니다. 내용이 저장 장치까지 실제로 내려갔는가, 파일 이름이 새 내용을 가리키게 됐는가, 그 둘 사이에서 전원이 끊기면 무엇이 남는가입니다. 이 절은 그 셋을 각각 어느 호출이 책임지는지 가릅니다.

### 1.1 일반 개념: rename 의 원자성

POSIX 와 Windows 모두 "같은 파일시스템 안에서의 이름 바꾸기"를 **원자적 연산**(쪼개지지 않는 한 걸음 — 하다 만 중간 상태가 바깥에서 보이지 않는 연산)으로 규정합니다. 외부 관찰자 입장에서 그 경로가 가리키는 대상은 "이전 파일" 아니면 "새 파일"입니다. 그 둘 사이의 **중간 상태가 존재하지 않습니다.**

> **💡 쉽게 말하면** — 벽에 붙은 안내문을 고치는 일과 같습니다. 붙어 있는 종이에
> 그대로 덧쓰면 지우다 만 상태를 지나가던 사람이 봅니다. 새 종이에 다 쓴 뒤 압정을
> 뽑아 한 번에 갈아 끼우면, 보는 사람에게는 옛 안내문 아니면 새 안내문만 보입니다.
> 이름 바꾸기가 그 "압정 뽑고 갈아 끼우기" 한 동작에 해당합니다.
> 다만 이 비유는 **같은 벽 안에서만** 성립합니다 — 다른 파일시스템(예를 들어 다른
> 드라이브)으로는 갈아 끼우기 자체가 되지 않습니다. 압정만 뽑아서는 옮길 수 없듯이
> `os.replace` 가 `OSError`(EXDEV)로 그냥 실패하고, 옛 안내문은 벽에 그대로 남습니다.
> 찢어진 중간 상태가 생기는 게 아니라 아무것도 일어나지 않는 쪽입니다.
...
> **💡 쉽게 말하면** — 편지를 부치는 일에 빗댈 수 있습니다. 편지를 다 썼습니다
> (`f.write`). 봉투에 넣어 현관 앞에 내놨습니다(`f.flush`). 우체통에 실제로
> 넣었습니다(`os.fsync`). 그리고 받는 사람 이름표를 새것으로 갈아 끼웠습니다
> (`os.replace`). 정전은 현관에 둔 편지를 태웁니다 — 그래서 우체통까지 가는 단계를
> 건너뛸 수 없습니다.
> 다만 이 비유는 **순서**에서 깨집니다 — 편지는 부치고 나면 끝이지만, 여기서는
> 우체통에 넣는 일(`fsync`)이 이름표를 가는 일(`os.replace`)보다 반드시 **먼저**
> 와야 합니다. 순서가 뒤집힌 경우가 아래 시나리오 D 입니다.
...
프로그램이 하던 일을 끝내지 못하고 갑자기 멈추는 순간(crash)을 네 지점으로 나눠 봅니다. 각 상자는 "그때 파일이 어떤 상태였고, 다시 켜면 무엇이 남아 있는가"를 적은 것입니다.
...
`fsync(파일)` 이 내려 보내는 것은 **그 파일의 내용과 그 파일의 메타데이터**(크기·수정 시각처럼 내용이 아닌 부속 정보)입니다. 그런데 "`transactions.jsonl` 이라는 이름이 어느 inode(파일 실체에 붙은 번호표 — 이름과 실체는 따로 관리됩니다)를 가리키는가"는 파일이 아니라 **부모 디렉터리**에 적힌 정보입니다. `os.replace` 가 바꾸는 것이 바로 그 디렉터리 엔트리이고, 그 변경은 아직 디스크에 내려가지 않았을 수 있습니다.

> **💡 쉽게 말하면** — 서류 자체는 금고에 넣어 잠갔는데, "그 서류가 몇 번 서랍에
> 있다"고 적어 둔 색인 카드는 아직 책상 위에 놓여 있는 상태입니다. 서류는 무사하지만
> 그것을 찾아가는 길이 아직 확정되지 않았습니다.
> 다만 이 비유는 **손해의 크기**에서 깨집니다 — 색인 카드를 잃으면 서류를 영영 못
> 찾지만, 여기서는 옛 이름이 옛 파일을 그대로 가리키므로 대개 "이번 변경만 취소된"
> 상태로 끝납니다(바로 아래에 적은 최악의 경우는 예외입니다).
...
**불변식**이란 언제 확인해도 반드시 참이어야 하는 약속을 말합니다. 이 절은 이 프로젝트가 **파일 상태에 대해 보장하는 것** 셋을 명시합니다. 셋 다 리팩터에서 세워졌습니다.

> **💡 쉽게 말하면** — 가게 문에 "영수증은 절대 버리지 않습니다"라고 써 붙이는 것과
> 같습니다. 손님이 아무 때 들어와 확인해도 그 문장이 참이어야 하고, 한 번이라도
> 거짓이면 그것은 사소한 예외가 아니라 약속이 깨진 것입니다.
> 다만 이 비유는 **약속의 범위**에서 깨집니다 — 여기서 약속하는 주체는 이 프로그램
> 하나뿐입니다. 사람이 텍스트 에디터로 파일을 직접 고치는 것까지는 막지 못합니다.

### 2.1 불변식 1 — "해석하지 못한 줄은 사라지지 않는다"

**보장 내용**: 어떤 쓰기 연산도 파싱(글자 뭉치를 프로그램이 다룰 수 있는 값으로 해석하는 일)에 실패한 줄을 삭제하지 않습니다.

> **💡 쉽게 말하면** — 물에 젖어 글씨가 번진 줄이 섞인 장부를 새 장부로 옮겨 적는
> 일입니다. 못 읽겠다고 그 줄을 빼 버리면, 원본을 버리는 순간 그 내역은 세상에서
> 사라집니다. 못 읽는 모양 그대로라도 베껴 두면 나중에 사람이 알아볼 여지가 남습니다.
...
거기에 **파일 스캔만으로는 부족한 축이 하나 더** 있습니다. 스캔 최대값은 삭제하면 줄어들어 지운 번호가 부활하므로, `id_allocator()` 는 스캔 최대값과 **워터마크 파일**(한 번 올라가면 내려오지 않는 수위 표시 — 여기서는 "지금까지 발급한 적 있는 가장 큰 번호"입니다. `data/id_counter`, `IdWatermark`)의 **최대값**에서 시작합니다(storage/repositories.py:65-77). 워터마크는 줄어들지 않습니다.
...
CSV 파일을 통째로 가져올 때 그 안에 잘못된 줄이 섞여 있으면 어떻게 할 것인가를 정하는 절입니다. 답은 하나가 아니라 두 갈래 질문으로 나뉩니다 — "잘못된 줄을 만나면 하던 일을 전부 무를 것인가"(이 '전부 아니면 전무'를 원자성이라고 부릅니다 — 트랜잭션이 보장하는 성질 가운데 하나일 뿐이고, 나머지 하나인 '동시에 손대도 안전한가'는 §6.2 에서 따로 봅니다), 그리고 "이미 저장돼 있는 거래를 또 만나면 어떻게 할 것인가".
...
> **💡 쉽게 말하면** — 계약서 두 장에 도장을 찍어야 하는 일입니다. 손은 하나뿐이라
> 아무리 빨리 찍어도 "첫 장만 찍힌 순간"이 반드시 존재합니다. 준비를 미리 다 해 두면
> 그 순간을 아주 짧게 줄일 수는 있어도 없앨 수는 없습니다. 그 순간까지 없애려면
> "둘 다 찍힌 것으로 친다"를 대신 기록해 줄 장치가 따로 있어야 하는데, 데이터베이스
> 세계에서 그 역할을 하는 것이 저널이나 SQLite 입니다.
> 다만 이 비유는 **그 장치의 정체**에서 깨집니다 — 저널은 바깥의 제삼자가 아니라
> "무엇을 하려던 중이었는지"를 미리 적어 두는 또 하나의 파일이고, 중단됐을 때 그
> 기록을 보고 마저 끝내거나 되돌리는 절차가 반드시 함께 있어야 합니다.
...
명령 하나하나가 "자료가 늘어나면 얼마나 더 느려지는가"를 따져 봅니다. 지금 규모(수천 건)에서는 어느 명령이든 눈 깜짝할 새에 끝납니다. 그러니 이 절은 느린 곳을 당장 고치자는 이야기가 아니라, **무엇이 가장 먼저 무너질지를 미리 알아 두자는 이야기**입니다.
...
> **💡 쉽게 말하면** — O(N) 은 "손님이 두 배로 늘면 걸리는 시간도 두 배"라는 뜻입니다.
> 줄 선 사람을 하나씩 세는 일이 그렇습니다. O(1) 은 "몇 명이 서 있든 걸리는 시간이
> 같다"는 뜻이고, 줄 맨 뒤에 한 사람을 더 세우는 일이 그렇습니다. O(N log N) 은
> O(N) 보다 **조금 더 비싼** 쪽입니다 — 정렬이 여기 속하는데, 손님을 한 번 세는 것으로
> 끝나지 않고 순서를 매기느라 몇 번 더 훑어야 하기 때문입니다. 정렬이 메모리까지
> O(N) 인 것은 또 다른 이유입니다 — 순서를 매기려면 일단 전부 모아 놓고 봐야 하니까요.
> 다만 이 비유는 **실제 걸리는 시간**에서 깨집니다 — 이 표기는 늘어나는 배수만 적을
> 뿐 "몇 초 걸리는가"는 말해 주지 않습니다. 그래서 자료가 적을 때는 O(N) 쪽이
> O(N log N) 쪽보다 느린 일도 얼마든지 있습니다.

**이 표를 읽는 법.** 각 칸의 `O(...)` 는 "자료가 N배로 늘면 비용이 대략 몇 배가 되는가"만 적은 것이지 초 단위 속도가 아닙니다. 굵게 표시된 칸이 이 표에서 눈여겨볼 자리이니, 표 전체를 해석하려 들지 말고 **굵은 칸과 표 바로 아래의 설명 세 문단만 읽어도 됩니다.**
...
**제너레이터 식을 그대로 넘깁니다.** 제너레이터란 결과를 미리 전부 만들어 두지 않고, 달라고 할 때마다 한 건씩 꺼내 주는 장치입니다. `csv_io.write_transactions` 가 `for tx in txs:` 로 하나씩 당겨 쓰므로, 100만 건을 내보내도 메모리는 한 건분입니다.
...
**공통 개선 방향은 "인덱스"**(책 뒤의 찾아보기처럼, 전부 훑지 않고 곧장 해당 자리로 가게 해 주는 별도 자료)이고, 그것을 제대로 하려면 **SQLite** 가 답입니다(§5.3).

---

## 5. 저장 포맷 트레이드오프

장부를 어떤 모양의 파일로 적어 둘 것인가를 고른 이야기입니다. 이 프로젝트는 **한 줄에 거래 하나씩** 적는 방식(JSONL)을 골랐습니다. 그것이 유일한 정답이어서가 아니라, 다른 후보 셋과 하나하나 견줘 보고 고른 것입니다.
...
**이 프로그램에는 파일 잠금이 없습니다.** 잠금이란 한쪽이 파일을 고치는 동안 다른 쪽이 손대지 못하게 걸어 두는 장치입니다. 그것이 없으므로, 터미널 창 두 개에서 이 프로그램을 동시에 실행하면(= 두 프로세스가 같은 파일을 함께 만지면) 문제가 생깁니다.
...
**워터마크 파일도 이것을 막지 못합니다.** `IdWatermark.remember` 자체가 "읽고(`read`) 비교하고 쓰는"(`atomic_write_lines`) read-modify-write(읽고 → 고치고 → 다시 쓰는 세 걸음) 이기 때문입니다(storage/ids.py:77-78). t1·t2 에 둘 다 5 를 읽었으면 둘 다 6 을 씁니다 — §6.2 의 lost update 와 **정확히 같은 구조**입니다. 워터마크가 푸는 문제는 "삭제로 번호가 되돌아가는 것"이지 "동시 실행"이 아닙니다.
...
**A 가 읽은 시점의 스냅숏(그 순간의 내용을 통째로 떠 둔 사본)으로 파일을 덮어쓰므로**, 그 사이 B 가 추가한 것이 유실됩니다. 이것을 **lost update** 라고 합니다.

> **💡 쉽게 말하면** — 두 사람이 같은 장부를 각자 한 부씩 베껴 갔다고 해 봅시다.
> 한 사람은 한 줄을 지웠고, 다른 사람은 한 줄을 더했습니다. 둘 다 자기가 베낀 종이를
> 원본 자리에 도로 갖다 놓으면 **뒤에 놓은 사람의 종이만 남습니다.** 앞사람의 수정은
> 경고 한 줄 없이 사라집니다.
> 다만 이 비유는 **누가 이기는가**에서 깨집니다 — 여기서 이기는 쪽은 "나중에 도착한
> 사람"이 아니라 "나중에 `os.replace` 를 실행한 쪽"입니다. 누가 먼저 읽기 시작했는지는
> 결과에 아무 영향이 없습니다.
...
**원자성(atomicity)과 격리성(isolation)은 다른 보장입니다.** 원자성은 "하다 만 상태가 남지 않는다"이고, 격리성은 "동시에 도는 둘이 서로의 작업 중간에 끼어들지 않는다"입니다. DB 의 ACID 에서 A 와 I 가 따로 있는 이유가 이것입니다. 이 프로그램은 A 를 (파일 하나에 대해) 갖고 있고 I 는 전혀 갖고 있지 않습니다. "`os.replace` 를 쓰니까 동시 실행도 안전합니다"는 **틀린 문장**이고, 방어 질문에서 이 구분을 못 하면 원자성을 이해했다고 보기 어렵습니다.
...
이 코드가 "이런 일이 생기면 어쩌지"를 미리 막아 둔 자리를 한곳에 모았습니다. 25개를 다 읽을 필요는 없습니다. 표를 훑다가 눈에 걸리는 항목만 오른쪽 위치를 따라가 보면 됩니다.
...
여기 적힌 것은 전부 **아직 만들지 않은 것**입니다. 표의 요점은 기능 목록이 아니라 오른쪽 칸입니다 — 그 기능을 붙이려면 프로그램의 **어느 한 덩어리만** 손대면 되는가를 적어 두었습니다.
...
이 문서 전체를 여섯 개의 질문과 답으로 줄인 절입니다. 앞을 다 읽지 않았더라도 여기부터 읽고, 걸리는 항목만 앞으로 되짚어 갈 수 있습니다.
