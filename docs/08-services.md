...
저장소와 CLI 사이에서 **판단**만 담당하는 계층입니다. 이 패키지의 어느 파일에도 `open()` 도 `print()` 도 없습니다 — `grep -rn "open(" budget_app/services/` 의 유일한 결과는 `services/__init__.py:4` 의 docstring 한 줄이고 `print(` 는 0건입니다. 검색·요약·카테고리 보호·CSV 가져오기의 정책을 다룹니다.
...
**모든 서비스가 저장소를 생성자로 받습니다** — 서비스가 `TransactionRepository` 를 스스로 만들지 않고 `__init__` 으로 건네받으므로, 테스트에서 가짜 저장소를 끼워 넣을 수 있습니다(의존성 주입 — [04 §6](./04-architecture.md)).
...
**저장소는 "없었다"는 사실만 보고, 서비스가 그것을 "오류"로 판정합니다.** 이 분리가 유용한 이유는 "id 가 없다"가 항상 오류인 것은 아니기 때문입니다 — 멱등 삭제를 추가한다면 저장소는 그대로 두고 서비스만 바꾸면 됩니다.
...
이 서비스가 지키는 규칙은 **"거래가 참조하는 카테고리는 사라지면 안 된다"** 입니다. 관계형 DB 의 외래 키 제약을 코드로 구현한 것과 같습니다.
...
**"스키마는 csv_io 가 안다"** 는 첫 문장이 경계를 선언합니다. 이 클래스에는 `"date"`, `"amount"` 같은 컬럼명이 **한 번도 등장하지 않습니다.**
...
`include_id=True` 가 기본인 이유가 이 리팩터의 출발점이었습니다 — id 컬럼이 없으면 내보낸 CSV 를 다시 넣을 때 모든 행이 새 id 를 받아 같은 거래가 두 벌이 됩니다. 자세한 버그 재현은 [07 §8.2](./07-repository.md)에 있습니다.
...
**루프 시작 전에 파일을 두 번 훑습니다** — `id_allocator()`(거래 파일)와 `name_set()`(카테고리 파일). 이 두 스냅숏 덕분에 **행마다 파일을 다시 읽지 않습니다.**
...
**원자 모드는 `UnitOfWork` 로 두 파일을 한 단위로 커밋합니다.**
...
`--atomic` 과 `--on-duplicate` 는 **다른 것을 다룹니다.**
...
**부수 효과까지 롤백됩니다.** 카테고리 자동 등록도 커밋 단계에 있으므로, 준비 중 실패하면 카테고리도 남지 않습니다. 실제로 검증한 결과:
...
**판단**입니다. 파일을 열지 않고(저장소가 함) 글자를 내지 않으며(프레젠터가 함), "카테고리가 등록되어 있는가", "사용 중인 카테고리를 지워도 되는가", "중복 id 를 어떻게 처리할까" 같은 정책만 결정합니다. `budget_app/services/` 전체에 `open(` 이 `__init__.py:4` 의 docstring 한 줄 외에 없고 `print(` 는 0건이라는 것으로 검증됩니다.
