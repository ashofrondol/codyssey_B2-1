...
이 프로그램은 터미널에서 쓰는 가계부입니다. 명령을 한 줄 치면 수입·지출 한 건이 기록되고, 달마다 얼마를 벌고 썼는지 셈해서 보여 줍니다. 이런 일을 하는 코드가 한 덩어리로 뭉쳐 있으면, 화면 글자 하나를 고치려다 돈 계산이 틀어지는 사고가 납니다. 그래서 이 프로그램은 일을 네 구역으로 나눠 두었습니다 — 사용자의 말을 받는 곳, 규칙에 맞는지 따지는 곳, 파일에 적고 꺼내 오는 곳, "거래 한 건이란 무엇인가"를 정해 두는 곳입니다. 식당으로 치면 홀·주방·창고·조리법에 해당합니다. 이 문서는 그 구역 나누기가 그림으로만 있는 것이 아니라 실제 코드에서 지켜지고 있는지를 확인하고, 명령 하나가 네 구역을 어떤 순서로 지나 파일에 닿는지를 처음부터 끝까지 따라갑니다. 까다로운 대목은 여기입니다 — 구역은 선을 그어 놓는다고 저절로 지켜지지 않고, 코드가 그 선을 넘지 않는지 누군가 계속 확인해야만 유지됩니다.

**이 문서에 자주 나오는 말**

| 말 | 쉬운 뜻 |
| --- | --- |
| 계층 | 하는 일의 성격에 따라 코드를 위아래로 나눠 놓은 구역. 이 프로그램은 넷(CLI·서비스·저장소·도메인)입니다 |
| 의존 / import | A 가 B 없이는 돌아가지 않는 관계를 "A 가 B 에 의존한다"고 하고, 코드에서는 `import` 한 줄이 그 증거입니다. 이 화살표가 위에서 아래로만 향하게 두는 것이 이 문서의 핵심 규칙입니다 |
| 순환 import | 두 파일이 서로를 import 해서, 한쪽이 아직 준비를 끝내지 못한 반쪽짜리 상태로 상대에게 건네지는 것. 파이썬은 여기서 멈추거나 기다리지 않습니다 — 운이 좋으면 그대로 돌아가고, 나쁘면 `ImportError` 가 납니다 |
| 합성 루트 | 프로그램이 시작할 때 그 명령이 쓸 저장소·서비스를 한자리에서 만들어 끼워 주는 곳. 여기서는 `AppContext` 입니다 |
| 의존성 주입(DI) | 필요한 부품을 스스로 만들지 않고 밖에서 받는 것. 그래야 시험할 때 가짜 부품으로 바꿔 끼울 수 있습니다 |
| 데코레이터 | 함수 위에 `@이름` 을 붙여, 원래 함수는 그대로 둔 채 겉에 일을 한 겹 덧씌우는 파이썬 문법 |
| 원자적 | 바꾸다 만 중간 상태가 남지 않고, "바꾸기 전" 아니면 "바꾼 후"만 존재하는 것 |
| 종료 코드 | 프로그램이 끝나면서 남기는 숫자. 0 이면 성공이고, 다른 숫자는 실패의 종류를 뜻합니다 |

**바쁘면 여기만**

- **§1 전체 그림 — 계층 다이어그램** — 코드 전체가 어떤 구역으로 나뉘어 있는지가 그림 한 장에 담겨 있습니다. 이것만 봐도 나머지를 읽을 지도가 생깁니다.
- **§3·§4·§5 실행 흐름 완전 추적 3종** — 명령을 하나 쳤을 때 그것이 어디를 지나 파일에 닿는지를 손가락으로 짚어 갈 수 있게 적어 둔 부분입니다. 쓰기(§3)·읽기(§4)·가장 복잡한 가져오기(§5) 셋이라, 하나만 골라 읽어도 됩니다.
- **§9 설계의 진화 — 리팩터 전후 대조표** — 무엇이 잘못돼 있었고 어떻게 고쳤는지가 표 하나에 모여 있습니다. "왜 하필 이 모양인가"의 답이 여기 있습니다.

---

이 문서는 budget_app 의 전체 지도입니다. 왜 코드가 43개 파일(구현 38 + 패키지 선언 `__init__.py` 5)로 나뉘어 있는지, 명령 하나가 파일 저장까지 어떤 경로를 지나가는지를 코드로 실증합니다. 그 구조가 어떤 설계 원칙과 리팩터링 과정을 거쳐 만들어졌는지도 함께 따라갑니다.
...
budget_app 은 일을 네 구역으로 나눠 두었습니다 — 명령을 받는 곳(CLI), 규칙을 판단하는 곳(서비스), 파일을 읽고 쓰는 곳(저장소), 데이터가 무엇인지 정의하는 곳(도메인). 그리고 그 순서대로 **한 방향으로만 의존합니다.**

> **💡 쉽게 말하면** — 식당을 떠올리면 됩니다. 홀 직원은 손님의 주문을 받아 주방에 넘기고, 주방은 "이 재료로 이 요리가 되는가"를 판단해 창고에서 재료를 꺼내 쓰고, 조리법 책은 "이 요리가 무엇인가"만 적어 둡니다. 홀을 통째로 개조해도 창고는 하던 대로 하면 됩니다 — 화살표가 아래로만 향하니 위를 바꿔도 아래는 그대로인 것이고, 구역을 나누는 값어치가 여기 있습니다. (반대로 창고가 재료를 내주는 방식을 바꾸면 그것을 받아 쓰는 주방은 따라 바뀝니다.)
> 다만 이 비유는 사람이 급하면 선을 넘을 수 있다는 데서 깨집니다 — 홀 직원은 바쁘면 창고에 직접 뛰어들 수 있지만, 이 코드에서는 `cli/` 가 `storage/` 를 **import 하는 순간** 테스트가 실패합니다(§1.2.2).
...
> **💡 쉽게 말하면** — 창고 담당자는 오늘 홀에 손님이 몇 명 앉아 있는지 알 필요가 없습니다. 모르는 편이 낫기도 합니다 — 홀을 카페처럼 개조해도 창고는 하던 대로 하면 되니까요. 코드에서 "모른다"는 그 파일 이름을 한 번도 쓰지 않는다는 뜻이고, 그래서 위쪽을 통째로 갈아 끼워도 아래쪽에는 손댈 곳이 없습니다.
> 다만 이 비유는 "몰라도 일이 된다"까지가 한계입니다 — 창고에 물건이 없으면 홀도 결국 멈추듯, 아래 계층이 고장 나면 위 계층도 함께 멈춥니다. 모르는 것은 상대의 **정체**이지 상대의 **사정**이 아닙니다.
...
지금은 각각 `cli/parser.py` / `cli/prompts.py` / `cli/presenter.py` / `cli/handlers.py` 로 나뉘어 파일마다 이유가 하나입니다(`cli/app.py` 는 그 넷을 엮는 레지스트리(이름과 함수를 짝지어 둔 목록)와 진입점만 남았습니다). 파일 크기도 함께 줄었습니다.
...
> **💡 쉽게 말하면** — 이사한 A 가 방에서 짐을 풀다가 드라이버가 필요해 옆방의 B 를 부릅니다. **A 가 불렀기 때문에** B 가 그제야 자기 짐을 풀기 시작하는데, B 도 A 의 드라이버가 있어야 해서 A 의 방으로 돌아옵니다 — 그런데 A 의 방은 아직 반쯤만 풀린 채라 그 드라이버가 상자 밖으로 나와 있지 않습니다. 둘이 동시에 엇갈린 것이 아니라 `A → B → A` 한 줄기로 되돌아온 것입니다. 두 파일이 서로를 import 할 때 파이썬이 겪는 일이 정확히 이것이고, 아래 ⚙️ 상자의 "반쯤 채워진 모듈"이 아직 상자를 다 못 푼 A 의 방입니다.
> 다만 이 비유는 사람이라면 잠시 기다렸다 다시 오면 된다는 데서 깨집니다 — 파이썬은 A 를 처음부터 다시 실행해 주지 않고 반쯤 푼 그 방을 그대로 건네주므로, 기다려서 해결되지 않습니다. 그래서 필요한 이름이 이미 나와 있으면 그냥 넘어가고, 아직이면 그 자리에서 `ImportError` 가 납니다.
...
> **💡 쉽게 말하면** — 주차 금지를 지키게 하는 방법은 둘입니다. 벽에 "주차 금지" 안내문을 붙이거나, 차가 못 들어오게 말뚝을 박거나. 문서에 적어 둔 계층 규칙은 안내문이고, `tests/test_architecture.py` 는 말뚝입니다 — 규칙을 어긴 코드가 들어오면 테스트가 빨간불을 냅니다.
> 다만 이 비유는 말뚝이 물리적으로 막는다는 데서 깨집니다 — 테스트는 돌려야 알려 주므로, 아무도 실행하지 않으면 위반은 그대로 들어옵니다.
...
핵심은 판정을 **문자열 검색이 아니라 AST 로** 한다는 점입니다(AST — 파이썬이 코드를 읽어 만들어 두는 구문 트리. 글자가 아니라 문법 구조를 보므로 "진짜 import"만 셀 수 있습니다). 테스트 docstring 이 그 이유를 적어 뒀습니다.
...
이 구조가 처음부터 이랬던 것은 아닙니다. 리팩터 전에는 **아래 계층이 위 계층에 전이 의존**(직접 부르지는 않지만, 부른 것이 또 부르는 식으로 결국 이어지는 의존)하는 경로가 있었습니다.
...
`TransactionRepository` 가 이것을 재정의하지만, 더하는 일도 도메인 판단이 아니라 **ID 워터마크 갱신**(워터마크 — 지금까지 발급한 번호 중 가장 큰 값을 적어 두는 기준선)뿐입니다.
...
> **💡 쉽게 말하면** — `@handle_errors` 같은 데코레이터는 편지를 봉투에 넣는 일에 가깝습니다. 편지(원래 함수)는 한 글자도 바뀌지 않는데, 겉에 주소와 우표가 붙어야 비로소 우체국이 다룰 수 있는 물건이 됩니다. 여기서는 `_dispatch` 라는 이름이 가리키는 것이 이제 편지가 아니라 봉투이고, 안에서 무슨 사고가 나든 봉투가 그것을 받아 `[오류]` 한 줄과 종료 코드로 바꿔 내놓습니다.
> 다만 이 비유는 봉투가 속을 건드리지 않는다는 데서 깨집니다 — 데코레이터는 안의 함수가 던진 예외를 가로채 아예 다른 값을 대신 돌려줄 수 있습니다. 겉만 한 겹 두르고 마는 것이 아닙니다.
...
여기서 **CLI 가 자체 검증 규칙을 갖지 않는다**는 점이 중요합니다. `validators.parse_date` 를 그대로 넘길 뿐입니다. 카테고리만 서비스에 물어봐야 하므로 클로저 팩토리(어떤 값을 기억한 채로 새 함수를 만들어 내는 함수)로 감쌉니다 — `registered_category_validator(cat_service)` 는 `cat_service` 를 기억하는 **한 인자짜리 함수**를 만들어 돌려주므로, `ask_until` 이 요구하는 `Callable[[str], T]` 모양에 맞아떨어집니다(`cli/prompts.py:77-109`, 자세한 것은 [03 §4.5](./03-python-advanced.md)).
...
**7단계 — 생성자 불변식.** 불변식이란 그 객체가 존재하는 한 언제나 참이어야 하는 조건입니다. `Transaction(...)` 호출이 `__post_init__` 을 태우고, 여기서 모든 필드가 검증·정규화됩니다. 여기까지 예외 없이 왔다면 **객체는 반드시 유효**합니다.
...
이번에는 파일을 바꾸지 않고 읽기만 하는 경로입니다. "2024년 1월에 얼마를 벌고 얼마를 썼나"라는 물음이, 저장된 거래를 훑어 답을 만들고 화면까지 오는 길을 따라갑니다.
...
> **💡 쉽게 말하면** — 요리사가 자기 칼을 직접 벼려서만 쓴다면 평생 그 칼 하나에 묶입니다. 칼을 밖에서 받아 쓰기로 하면, 연습 때는 무딘 칼을 실전에는 잘 드는 칼을 쥐여 줄 수 있습니다 — 이것이 의존성 주입입니다. 그럼 칼은 누가 챙기느냐, 개업 준비 시간에 한 사람이 주방과 창고 몫을 한자리에서 꺼내 배치합니다. 그 자리가 합성 루트이고, 여기서는 `AppContext` 입니다. 홀 몫(파서·프롬프트·프레젠터 같은 CLI 부품)은 여기서 손대지 않고 `cli/app.py` 가 따로 엮습니다 — `context.py` 는 `cli.*` 를 import 하지 않는다는 것이 이 프로젝트의 규칙이라(§2 표), 나중에 홀 대신 웹 화면을 붙여도 조립하는 자리는 그대로입니다.
> 다만 이 비유는 가게가 하루 종일 열려 있다는 데서 깨집니다 — 이 프로그램은 명령 하나가 끝나면 프로세스째 사라지므로, 조립은 명령을 칠 때마다 처음부터 다시 합니다.
...
무엇인가 잘못됐을 때 **어디서 알아채고 어디서 사용자에게 말하는가**도 계층 설계의 일부입니다. 이 프로그램은 알아채는 자리는 여럿이되, **예외를** 사용자용 문구와 종료 코드로 바꾸는 자리는 `@handle_errors` 한 곳으로 모아 두었습니다. (예외를 쓰지 않는 실패 경로 — 이를테면 §3 의 `cmd_add` 가 카테고리가 하나도 없을 때 곧장 안내 문구를 내보내는 자리 — 는 핸들러·프롬프트가 직접 말합니다.)
...
> **💡 쉽게 말하면** — 주방에서 밖으로 나가는 것이 둘입니다. 손님 앞에 놓는 접시(stdout)와 주방 게시판에 적어 두는 메모(stderr)입니다. 접시 위에 "오늘 양파가 떨어졌음" 쪽지를 얹어 내보내면 손님은 그 접시를 쓸 수 없습니다 — `list > out.txt` 로 데이터만 받으려는데 오류 문구가 섞여 들어가는 상황이 그것입니다.
> 다만 이 비유는 둘이 끝까지 따로 간다는 데서 깨집니다 — 셸에서 `2>&1` 로 두 채널을 다시 합칠 수 있고, 그때 순서가 뒤집히는 문제가 아래 ⚙️ 상자의 내용입니다.
...
한편 도메인 계층은 다른 방식으로 추상화를 씁니다. `domain/specs.py` 의 `Spec` 은 `is_satisfied_by` 하나를 요구하는 기반 클래스이고, `And`/`Or`/`Not`/`DateFrom`/… 이 이를 구현해 조건을 조합 가능한 값으로 만듭니다. 같은 이름의 호출이 대상에 따라 다르게 동작하는 성질을 다형성이라고 하는데, 즉 **이 코드에 다형성이 없는 것이 아니라, 저장소 축에만 없습니다.**
...
구현 파일 38개(패키지 선언 5개 별도), 4계층입니다. 기준은 "이 파일을 고칠 이유가 몇 개인가"입니다. CLI 계층만 해도 문법(`parser`)·입력(`prompts`)·표시(`presenter`)·채널(`output`)·오류표현(`error_handler`)·오케스트레이션(부품들을 순서대로 불러 엮는 일, `handlers`)·조립과 진입(`app`)으로 나뉘는데, 각각 바뀌는 이유가 다르기 때문입니다. 의존 방향은 import 문으로 검증 가능하며(§1.2) 위로 향하는 의존이 하나도 없고, 그것을 `tests/test_architecture.py` 가 AST 로 매 실행마다 확인합니다.
